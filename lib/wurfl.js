...
function get(userAgent, deepcopy, notEqual) {
  ...
  if(deepcopy && typeof(device) !== 'undefined') {
    device = device.deepCopy(1);
  }
  if (typeof(device)!= 'undefined' && device.product_info){
  	console.log('going to find product_info');
  		device.product_info = findProductInfo(device);
  		console.log(device.product_info);
  		//console.log(device);
  		if (Object.keys(device.product_info).length <= 0){
  			device = null;
  		}else if (typeof (device.product_info.is_tablet) == 'undefined' || device.product_info.is_tablet==false	){
  			device = null;
  		}
  }
	return device;
...
function findProductInfo(device){
	...
	if (typeof(device) != 'undefined' && typeof(device.product_info)!='undefined' && !device.product_info.is_tablet){
		...
		return merge(device.product_info,findProductInfo(allDevices[device.fall_back]));
	}else if (typeof(device) != 'undefined' && typeof(device.product_info)!='undefined' && device.product_info.is_tablet){
		return device.product_info;
	}
	return product_info;
}
